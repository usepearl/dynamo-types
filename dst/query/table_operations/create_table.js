...
async function createTable(metadata) {
    let KeySchema;
    ...
    const params = {
         ...
         */
        KeySchema,
        LocalSecondaryIndexes: metadata.localSecondaryIndexes.length > 0 ?
            metadata.localSecondaryIndexes.map((index) => {
                return {
        ...
        GlobalSecondaryIndexes: metadata.globalSecondaryIndexes.length > 0 ?
            metadata.globalSecondaryIndexes.map((index) => {
                if (index.type === "FULL") {
                    return {
                        IndexName: index.name,
